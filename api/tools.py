...
def _screen_articles_llm(
    ...
    for batch_start in range(0, len(articles), SCREENING_BATCH_SIZE):
        ...
        prompt = (
            "You are a Pharmacovigilance Literature Screener performing PHASE 1 RELEVANCE SCREENING.\n\n"
            f"Investigation Context:\n{investigation_context}\n\n"
            f"Review the following {len(batch)} candidate articles.\n\n"
            "⚠️ YOUR TASK IS RELEVANCE SCREENING ONLY — NOT NOVELTY ASSESSMENT.\n"
            "Include an article even if the adverse event is already known or documented in the drug label.\n"
            "Novelty and clinical significance are evaluated in a separate downstream phase — do NOT apply novelty criteria here.\n\n"
            ...
            "- HUMAN subjects only (clinical trial, observational cohort, case report, meta-analysis, pharmacovigilance database study)\n"
            "- The investigated drug (or its generics / brand names / pharmacological class) is a PRIMARY safety exposure — not merely a co-medication or passing mention\n"
            "- The article reports ANY adverse event, safety finding, tolerability outcome, or drug interaction relevant to the drug in the investigation context — regardless of whether that AE is already labeled or well-known\n\n"
            ...
            "- Narrative review, editorial, or commentary with zero original patient data (systematic reviews with primary data are OK)\n"
            "- Article's primary topic is a completely unrelated drug or unrelated therapeutic area\n\n"
            "SCOPE: ICH E2D — human pharmacovigilance data only.\n\n"
            f"Articles:\n{articles_block}\n\n"
            "Respond with a JSON array for ONLY the relevant articles:\n"
            '[{"pmid": "12345678", "reason": "One sentence explaining relevance to the drug and an adverse event"}, ...]\n'
...
def _extract_article_summaries(
    ...
    for i, art in enumerate(articles):
        ...
        prompt = (
            "You are a strict Pharmacovigilance Literature Analyst operating under ICH E2D standards.\n\n"
            f"Investigation Context:\n{investigation_context}\n\n"
            f"=== ARTICLE | PMID: {pmid} ===\n"
            f"Title: {title}\n"
            f"Abstract: {abstract}\n"
            ...
            "TITLE vs ABSTRACT RULE: The ABSTRACT determines drug identity — NOT the title.\n"
            "A title may use class language ('ACE inhibitor-induced angioedema') while the abstract names a specific different drug.\n"
            "Always read the abstract carefully. If the abstract names a specific different drug as the primary study subject, set relevant=false — even if the title uses class language.\n\n"
            "Set relevant=false if:\n"
            "  ✗ The abstract identifies a SPECIFIC DIFFERENT drug as the primary study drug — e.g., the abstract names enalapril or ramipril as the drug studied, even if the title says 'ACE inhibitor'. Class language in the title does NOT override a specific drug name in the abstract.\n"
            ...
            "  ✓ The pharmacological CLASS broadly (e.g., 'ACE inhibitors' without naming a specific different drug in the abstract) — class-level signals are included\n"
